...
func CheckCart(cart models.Cart) *models.Cart {
	...
	for _, product := range cart.Products {
		var cost float32
		err := db.QueryRow("SELECT price * discount * $3 FROM products INNER JOIN productstock USING(productid) WHERE identifier=$1 AND option=$2 AND quantity > $3", product.Identifier, product.Option, product.Quantity).Scan(&cost)
		if err == nil {
			checkedCart.Products = append(checkedCart.Products, product)
			checkedCart.Total += cost
		}
	}
